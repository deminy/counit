...
* Tests using PHPUnit's process isolation (attributes _#[RunInSeparateProcess]_ and _#[RunTestsInSeparateProcesses]_,
  ...
  no other coroutine can make progress meanwhile (`Helper::coroutineHookFlags()` deliberately excludes
  `SWOOLE_HOOK_PROC`, so the parent's child-process spawning and pipe reads are native and never yield). Measured
  downstream: a 17-test isolated class inside a concurrent run took 11.2s, versus 2.2s for the same tests
  de-isolated. Mixing a few isolated tests into a suite is fine; running a whole suite under `--process-isolation`
  defeats the purpose of _counit_.
  * This holds through the Composer bin proxy too. For an isolated test that **preserves global state**
    (_#[PreserveGlobalState(true)]_; the default is `false` on _PHPUnit_ 10+), _PHPUnit_ replays the parent's
    included files into the child. The replay drops the entry script, but the only Composer bin proxy it
    ...
    script: a brand-new _PHPUnit_ run inside the child (every such test errored with "Test was run in child process
    and ended unexpectedly", and when that inner run could re-discover the isolated test class it recursed,
    spawning children without bound). The `counit` script now registers its own resolved path (and the entry
    script's) in `$GLOBALS['__PHPUNIT_ISOLATION_EXCLUDE_LIST']`, which _PHPUnit_ honors when building the replay —
    merging with, never clobbering, entries registered by others. Only the entry script is excluded: the Composer
    autoloader and counit's own classes still reach the child, where `Helper::isCoroutineFriendly()` returns false
    and everything falls back to plain blocking _PHPUnit_ behavior, as before.
