...
| Process isolation | ✅ exact semantics for `#[RunInSeparateProcess]`, `#[RunTestsInSeparateProcesses]` and `--process-isolation` — including through the Composer bin proxy (`vendor/bin/counit`, how every consumer runs it): the `counit` entry script registers itself in PHPUnit's isolation exclude list, so a global-state-preserving isolated child no longer replays and re-executes it (formerly every such test errored with "ended unexpectedly", or worse — see notes). But no speedup, and each isolated test serializes the run | ✅ (annotations) — same exclude-list registration, under both of its PHPUnit spellings (`__PHPUNIT_ISOLATION_EXCLUDE_LIST` as of _PHPUnit_ 9.3, `__PHPUNIT_ISOLATION_BLACKLIST` since 8.0.0), and more load-bearing there: _PHPUnit_ 8/9 default `preserveGlobalState` to **true**, so formerly *any* isolated test run through `vendor/bin/counit` made the child re-run the whole suite, recursively — the run hung, spawning processes without bound |
...
* Tests using PHPUnit's process isolation (annotations _@runInSeparateProcess_ and _@runTestsInSeparateProcesses_, or
  option `--process-isolation`) behave and are counted exactly as under _PHPUnit_, but they gain nothing from this
  package: _PHPUnit_ runs each of them in a separate child process, which is plain non-coroutine PHP, so the test
  body there runs in blocking mode. Worse, such a test blocks the entire run for as long as its child process takes,
  since no other coroutine can make progress meanwhile (`Helper::coroutineHookFlags()` deliberately excludes
  `SWOOLE_HOOK_PROC`, so the parent's child-process spawning and pipe reads are native and never yield). Mixing a few
  isolated tests into a suite is fine; running a whole suite under `--process-isolation` defeats the purpose of
  _counit_.
  * This holds through the Composer bin proxy too. For an isolated test that **preserves global state** — the
    DEFAULT on _PHPUnit_ 8/9, so every isolated test without `@preserveGlobalState disabled` — _PHPUnit_ replays the
    parent's included files into the child. The replay drops the entry script, but the only Composer bin proxy it
    special-cases is _PHPUnit_'s own — run through `vendor/bin/counit`, the proxy is the entry script and counit's
    real binary is the second included file, so the child used to replay and re-execute the whole counit entry
    script: a brand-new _PHPUnit_ run inside the child, which re-discovered the isolated test class and spawned
    children of its own, recursively — the parent blocked forever on a child's stdout and the run hung, spawning
    processes without bound. The `counit` script now registers its own resolved path (and the entry script's) in
    _PHPUnit_'s isolation exclude list, under both of its spellings — `$GLOBALS['__PHPUNIT_ISOLATION_EXCLUDE_LIST']`
    as of _PHPUnit_ 9.3, `$GLOBALS['__PHPUNIT_ISOLATION_BLACKLIST']` before the rename (present since 8.0.0, so
