 ...
 * in between. Under counit the main coroutine never blocks during the test loop (each test hands
 ...
 * whatever real test's window happened to be open when their coroutine resumed (join paths) or
 * in this drain window -- making the aggregate match a blocking run exactly, with full
 * concurrency kept. Per-test ATTRIBUTION stays wrong by construction (drain lines land under the
 * synthetic id; lines run inside a joined test's window are attributed to that test) -- the
 * documented residual.
 *
 * The coverage object is reached through the public TestResult::getCodeCoverage(); everything is
 * fail-soft: no coverage configured, no captured TestResult, or changed PHPUnit internals simply
 * leave the report as PHPUnit produced it.
 *
...
final class Coverage
{
     ...
     * @var bool
     */
    private static $drainWindowOpen = false;
    ...
    public static function startDrainWindow(): void
    {
        try {
            ...
            $codeCoverage = Counit::$testResult->getCodeCoverage();
            ...
            $codeCoverage->start('counit-drain');
