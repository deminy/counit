 ...
 * Counit::create() joins the coroutine at its first yield (and skips the up-front assertion
 * credit -- the body completes before PHPUnit reads the count), so PHPUnit's pcntl_alarm() guard
 * times the real test duration and the timeout is reported natively -- risky verdict,
 * --fail-on-risky honored, no deferred end-of-run block. Run by the compatibility workflow with
